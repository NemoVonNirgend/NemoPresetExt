/**
 * Lorebook Manager
 * Creates and manages a dedicated NemoLore Guides lorebook with current-chat
 * "tracker" entries. Entries persist in the lorebook, but each payload is
 * stamped with the chat ID that produced it.
 *
 * The lorebook is always loaded (global) and entries use `constant: true`.
 * Metadata lets us detect stale entries from a different chat and tell the AI
 * to refresh them instead of treating them as durable NemoLore memory.
 *
 * Metadata format (first line of entry content):
 *   <!-- NLG:chatId=abc123:updated=1710500000000 -->
 *
...
const DEFAULT_BOOK_NAME = 'NemoLore-Trackers';

/** Prefix for all NemoLore Guides-managed lorebook entries. */
const ENTRY_PREFIX = '[NLG]';

/** Regex to extract metadata from entry content. */
const METADATA_REGEX = /^<!-- NLG:chatId=([^:]*):updated=(\d+) -->\n?/;

/** Tracker definitions - live scratchpad aspects stored as lorebook entries. */
...
function buildMetadata(chatId) {
    return `<!-- NLG:chatId=${chatId || 'unknown'}:updated=${Date.now()} -->`;
}
...
function getConfiguredBookName() {
    ...
    return settings?.lorebookName?.trim() || '';
}

export function getBookName() {
    return getConfiguredBookName() || DEFAULT_BOOK_NAME;
}
...
/**
 * Ensure the NemoLore tracker lorebook exists and is activated in global World Info.
...
/**
 * Auto-activate the NemoLore tracker lorebook in the global World Info selection.
...
/**
 * Wipe all NLG tracker entries for a fresh current-chat start.
...
export async function buildTrackerStatusMessage() {
    ...
    if (report.isNewChat) {
        ...
        lines.push(`\n[NLG STATUS: FRESH CHAT — Story rules, scene assessment, and DM notes are being initialized automatically. Use the tracker data in your lorebook context to inform your response.]`);
        return lines.join('');
    }
    ...
    if (needsUpdate.length > 0) {
        ...
        if (needsUpdate.length === Object.keys(TRACKERS).length) {
            lines.push(`\n[NLG STATUS: All scene trackers are outdated or empty. Use Scene Assessment to populate: ${labels.join(', ')}.]`);
        } else {
            lines.push(`\n[NLG STATUS: The following trackers need updating: ${labels.join(', ')}. Use Scene Assessment with the relevant aspects to refresh them.]`);
        }
    }

    if (report.fresh.length > 0) {
        const freshLabels = report.fresh.map(t => TRACKERS[t]?.label || t);
        lines.push(`\n[NLG STATUS: Current trackers (up-to-date): ${freshLabels.join(', ')}. Only update these if the scene state has changed.]`);
    }
