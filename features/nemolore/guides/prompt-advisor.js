...
const LOG_PREFIX = '[NemoLore:Guides:PromptAdvisor]';
const ADVISOR_TITLE = 'NemoLore Guides - Prompt Stack Advisor';
...
- Anti-slop/writing quality prompts — those are handled by NemoLore Guides' writing analyzer automatically. Skip anything that looks like a banned-words list, anti-repetition filter, or quality enforcer.
...
/**
 * Run the prompt stack advisor analysis.
...
function displayRecommendations(advice) {
    ...
    if (!hasRecommendations) {
        toastr.info('Your prompt setup looks good for this character! No changes recommended.', ADVISOR_TITLE, { timeOut: 8000 });
    }

    // Show the "Apply All" button if there are actionable recommendations
    if (hasRecommendations) {
        const applyBtn = document.getElementById('nlg_apply_recommendations');
...
export async function runAndStoreAdvice() {
    ...
    toastr.info('Analyzing your prompt setup...', 'NemoLore Guides', { timeOut: 3000 });

    const prompts = scanPrompts();
    if (prompts.length === 0) {
        toastr.warning('No prompts found in the prompt manager.', 'NemoLore Guides');
        return;
    }
    ...
    try {
        const result = await runSidecarGeneration({
            ...
            toolName: 'NLG_prompt_advisor',
            toolParams: { promptCount: prompts.length },
        });

        const jsonMatch = result.match(/\{[\s\S]*\}/);
        if (!jsonMatch) {
            toastr.warning('Prompt advisor returned an unparseable response.', 'NemoLore Guides');
            return;
        }
        ...
        toastr.error('Prompt advisor analysis failed.', 'NemoLore Guides');
    }
}
...
export async function applyAllRecommendations() {
    if (!lastAdvice) {
        toastr.warning('No recommendations to apply. Run the advisor first.', 'NemoLore Guides');
        return;
    }
    ...
    if (applied > 0) {
        toastr.success(`Applied ${applied} recommendation${applied > 1 ? 's' : ''}. Check your prompt list to verify.`, ADVISOR_TITLE, { timeOut: 8000 });
        // Hide the apply button
        const applyBtn = document.getElementById('nlg_apply_recommendations');
        if (applyBtn) applyBtn.style.display = 'none';
    } else {
        toastr.info('No changes were needed — prompts may have already been toggled.', 'NemoLore Guides');
    }
...
/**
 * Initialize the prompt stack advisor. Settings wiring lives in index.js.
 */
export function initPromptAdvisor() {
    // Reserved for future advisor hooks.
}
