 ...
 * when and how to use NemoLore Guides tools. User-editable in settings.
 */
export const DEFAULT_SYSTEM_INSTRUCTION = `[NemoLore Guides - Available Capabilities]
...
**DM Notes** — Your current-chat narrative scratchpad:
...
**Roll Dice** — Random tabletop outcomes:
- Use this when the scene calls for a random check, contest, damage roll, fortune roll, or other uncertain outcome.
- Supported formulas include d20, 1d20+5, 2d20kh1, 2d20kl1, and 4d6kh3.
- After receiving the roll result, incorporate it naturally into the next response instead of re-rolling.

**General guidelines:**
- For simple exchanges (casual conversation, brief actions), just respond naturally — no tools needed.
- When you choose to call a tool, call it directly. Do not narrate the tool choice, emit <think>/<cot> blocks, or write partial prose before the tool result.
...
- DM Notes should be updated after any significant story event — it's your live scratchpad for this chat.
- Tools build on each other — Scene Assessment results inform Plan & Refine, DM Notes inform everything, and Roll Dice resolves uncertain outcomes.
