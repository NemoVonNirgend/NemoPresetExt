 ...
 * Uses Connection Manager profiles when configured, with current-API fallback,
 * to run Rule Setup, Scene Assessment, and DM Notes in parallel.
 *
...
const LOG_PREFIX = '[NemoLore:Guides:Setup]';
...
function resolveToolProfileId(toolName, fallbackProfileId) {
    return String(getToolSettings(toolName)?.profileId || fallbackProfileId || '').trim();
}

async function runSetupRequest({ profileId, connectionId, systemPrompt, userPrompt, maxTokens }) {
    if (profileId) {
        const text = await runProfilePrompt(profileId, userPrompt, {
            systemPrompt,
            maxTokens,
            temperature: 0.7,
        });
    ...
    if (!connectionId) {
        throw new Error('No Connection Manager profile or current API connection is available.');
    }

    return await ApiRouter.send(
        connectionId,
...
export async function runFreshChatSetup() {
    ...
    if (needsCurrentApi && !connectionId) {
        console.warn(`${LOG_PREFIX} No profiles or current API connection available. Falling back to tool calls.`);
        return false;
    }
