 ...
 * Stored as a current-chat tracker entry for the active conversation.
 *
 * Can be triggered:
 *   - Automatically on first message of a new chat
 *   - Manually via the NLG_rule_setup tool call
...
export const TOOL_NAME = 'NLG_rule_setup';

export function getDefinition() {
    return {
        name: TOOL_NAME,
        displayName: 'Rule Setup',
        description: `Analyze the character card, scenario, and opening context to generate a supporting system prompt with story rules tailored to the genre, tone, and setting. This creates current-chat narrative guidelines that help maintain consistency throughout the conversation. Use this at the start of a new chat, or when the story's direction has shifted significantly and the rules need updating.`,
        parameters: {
            type: 'object',
            properties: {
                focus: {
...
async function execute({ focus, refresh } = {}) {
    ...
    if (narratorMatch) {
        ...
        console.log('[NemoLore:Guides] Saved narrator personality to separate tracker.');
    }

    // Store full rules as a current-chat tracker entry.
    const success = await updateTracker('rules', result);
    if (!success) {
        console.warn('[NemoLore:Guides] Could not write rules to lorebook tracker.');
    }
...
export async function autoGenerateRules() {
    ...
    console.log('[NemoLore:Guides] Auto-generating story rules for fresh chat...');
